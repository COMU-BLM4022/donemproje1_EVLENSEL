...
# Excel dosyasını okuyun ve 'melb_data_1' nesnesine atayın
melb_data_1 <- read_excel(here("melb_data_1.xlsx"))
...
Veri setimizi SuitCRM platformuna entegre ettik. Bu süreçte izlediğimiz adımlar aşağıdaki gibidir:

*SuitCRM Üzerinde Modül Oluşturma:*

-Öncelikle SuitCRM içerisinde veri setimizi ithal edeceğimiz bir modül oluşturduk. Bunun için SuitCRM'in "Module Builder" aracını kullandık.
-Yeni bir package (paket) oluşturduk ve bu paketin içerisine veri setimizin yapısına uygun bir modül ekledik.

*Field (Alan) Ayarlarını Yapma:*

-Oluşturduğumuz modül içerisinde veri setimizdeki sütunlara karşılık gelecek alanları (fields) tanımladık.
-Her bir alanın türünü (metin, sayı, tarih, vb.) veri setimizdeki verilerin türlerine uygun olacak şekilde ayarladık.

*CSV Dosyasının İthal Edilmesi:*

-SuitCRM'in "Import" işlevini kullanarak CSV dosyamızı platforma yükledik.
-İthal işlemi sırasında, CSV dosyasındaki sütunları, oluşturduğumuz modüldeki uygun alanlara eşleştirdik.
-İthalat süreci tamamlandıktan sonra, veri setimiz SuitCRM'deki "CustomDataModule" modülünde başarılı bir şekilde yer aldı.
-Bu süreçte herhangi bir kodlama gereksinimi olmadan, SuitCRM'in sağladığı kullanıcı arayüzü araçlarını kullanarak veri setimizi platforma entegre ettik. Böylece, veri setimiz SuitCRM üzerinden erişilebilir ve analiz edilebilir hale geldi.

-Veri ithalinin başarılı olduğunu ve verilerin doğru şekilde işlendiğini doğrulamak için SuitCRM'deki modül üzerinden verileri kontrol ettik. Tüm verilerin düzgün bir şekilde yer aldığını ve herhangi bir veri kaybı olmadığını teyit ettik.



## Q5 : Yapılması planlanan veri analizleri
...
file_path <- here("melb_data_1.xlsx")

# Train the model
model <- evlenselPackage::karar_agaci_train(file_path)

# Create a new data frame for prediction
new_data <- data.frame(
  CouncilArea = c("Yarra", "Yarra"),
  YearBuilt = c(2000, 2010),
  BuildingArea = c(150, 160),
  Landsize = c(200, 220),
  Car = c(1, 2),
  Bathroom = c(2, 3),
  Bedroom2 = c(3, 4),
...
# Predict prices
predictions <- evlenselPackage::karar_agaci_predict(model, new_data)
...
[Video](https://youtu.be/3PSjaOkwgCQ)
